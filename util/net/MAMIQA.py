...
from pyiqa.archs.brisque_arch import  *
from functools import partial
...
class MAMIQA(nn.Module):
    def __init__(self):
        super().__init__() 
        # self.net  = VAN(
        # embed_dims=[64, 128, 320, 512], mlp_ratios=[8, 8, 4, 4],
        # norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 3, 12, 3]) 
        self.net  = van_b2(pretrained=True, num_classes=1)
    ...
    def forward(self, batch):
        ...
        if self.training:
            ...
            x = random_crop(x, crop_size=224, crop_num=8)
            nss=nss.repeat(8,1)
        ...
        scores=self.net([x,nss]).reshape(bsz,-1)  .mean(dim=1)
